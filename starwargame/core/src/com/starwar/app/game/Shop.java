...
public class Shop extends Group {
    ...
    public Shop(Hero hero) {
        ...
        final TextButton btnMagneticField = new TextButton("Magnet", textButtonStyle);

        btnMagneticField.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                if (hero.isMoneyEnough(Hero.Skill.MAGNETIC.cost)) {
                    if (hero.upgrade(Hero.Skill.MAGNETIC)) {
                        hero.decreaseMoney(Hero.Skill.MAGNETIC.cost);
                    }
                }
            }
        });

        btnMagneticField.setPosition(20, 10);
        this.addActor(btnMagneticField);
